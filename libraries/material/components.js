...
prx.types.material_textfield = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		if(prx.editor) {
			...
			cR += '<div class="faux-input placeholder-input liveUpdate-placeholderColor changeProperty-textColor changeProperty-borderColor">'+prx.componentsHelper.getProp(item.placeholder,'other')+'</div>';
			cR += '</div>';
		}
		else {
			if(prx.componentsHelper.getProp(item.hasIcon,'boolean') && item.imgSrc.fileid != '') {
...
prx.types.material_onoffswitch = {
	...
	,onDisplay: function(item,containerid,symbol) {
        ...
        cR += '</span>';
		cR += '<span class="material-onoff-switch '+((prx.componentsHelper.getProp(item.active,'boolean')) ? 'liveUpdate-activeSwitchColor changeProperty-activeSwitchColor' : 'liveUpdate-inactiveSwitchColor changeProperty-inactiveSwitchColor')+'">';
